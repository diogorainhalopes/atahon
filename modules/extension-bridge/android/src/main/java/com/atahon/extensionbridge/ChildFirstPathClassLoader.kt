package com.atahon.extensionbridge

import dalvik.system.PathClassLoader
import java.io.IOException
import java.io.InputStream
import java.net.URL
import java.util.Enumeration

/**
 * A parent-last class loader that tries in order:
 *   1. Already-loaded class cache (findLoadedClass)
 *   2. System class loader (android.*, java.*, etc.)
 *   3. This loader's own dex (the extension APK)
 *   4. Parent class loader (host app — provides OkHttp, Jsoup, RxJava, etc.)
 *
 * This mirrors Mihon's ChildFirstPathClassLoader exactly, ensuring extension code
 * takes priority over the host app while still sharing the host's network/parse deps.
 */
class ChildFirstPathClassLoader(
    dexPath: String,
    librarySearchPath: String?,
    parent: ClassLoader,
) : PathClassLoader(dexPath, librarySearchPath, parent) {

    private val systemClassLoader: ClassLoader? = ClassLoader.getSystemClassLoader()

    override fun loadClass(name: String?, resolve: Boolean): Class<*> {
        // 1. Already cached?
        var c: Class<*>? = findLoadedClass(name)

        // 2. System classes (android.*, java.*, etc.)
        if (c == null) {
            try {
                c = systemClassLoader?.loadClass(name)
            } catch (_: ClassNotFoundException) {}
        }

        // 3. Extension's own dex, then fall back to parent (host app)
        if (c == null) {
            c = try {
                findClass(name)
            } catch (_: ClassNotFoundException) {
                super.loadClass(name, resolve)
            }
        }

        if (resolve) resolveClass(c)
        return c!!
    }

    override fun getResource(name: String?): URL? {
        return systemClassLoader?.getResource(name)
            ?: findResource(name)
            ?: super.getResource(name)
    }

    override fun getResources(name: String?): Enumeration<URL> {
        val systemUrls = systemClassLoader?.getResources(name)
        val localUrls = findResources(name)
        val parentUrls = parent?.getResources(name)
        val urls = buildList {
            while (systemUrls?.hasMoreElements() == true) add(systemUrls.nextElement())
            while (localUrls?.hasMoreElements() == true) add(localUrls.nextElement())
            while (parentUrls?.hasMoreElements() == true) add(parentUrls.nextElement())
        }
        return object : Enumeration<URL> {
            val iterator = urls.iterator()
            override fun hasMoreElements() = iterator.hasNext()
            override fun nextElement() = iterator.next()
        }
    }

    override fun getResourceAsStream(name: String?): InputStream? {
        return try {
            getResource(name)?.openStream()
        } catch (_: IOException) {
            null
        }
    }
}

...
- Is loaded into the host app's process via a dedicated child-first classloader

<Callout type="warning">
  Extensions are **not sandboxed**. They run inside Atahon's own process with the same Android permissions and filesystem access as the app — they can make any network request the app can make. In practice well-behaved extensions only respond to source API calls (search, fetch chapter, fetch pages), but nothing technically prevents them from doing more. Only install extensions from repositories you trust.
</Callout>
